...
- The domain name is giving by netlify if we swap to readthedocs, the URL will find a broken webpage. 
- If we buy one, we will redirect trafic from one to another url. While we could redirect from netlify to readthedocs, it is much better to type theturingway.org (or the prefferable domain). We don't need to rely on netlify or readthedocs.
...
KW:
- 2022/23 there was a lot that the community wants this without actual proposal  ---> Encourage working groups to prepare proper request, to have more different pathways 
