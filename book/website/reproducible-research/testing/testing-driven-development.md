...
One way of ensuring tests are not neglected in a project is to adopt {term}`test-driven development <test driven development>`.
This is an approach in which {term}`unit tests <unit testing>` are written before the code.
...
{term}`Test driven development <test driven development>` forces you to break up your code into small discrete units, to make them easier to test; the code must be modular.
The benefits of this were discussed in the section on {ref}`unit testing<rr-testing-unittest>`.

An alternative development approach is {term}`behaviour-driven development <behaviour driven development>`.
...
In such cases {term}`behaviour-driven development <behaviour driven development>` is a path worth considering.
