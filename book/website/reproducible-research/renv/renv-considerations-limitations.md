...
This chapter covers a number of different tools technologies for describing, capturing and sharing computational environments.
...
A virtual machine image is a highly complete way to capture a computation environment but in the absence of a description of how it was built it is not particularly verifiable.
...
It is also not uncommon to be relying on various resources from the internet during such a build and what is at the other end of a URL can change or disappear making from one build to the next.
...
For this to be possible the dependency tree must be a directed acyclic graph, you cannot have a piece of software that indirectly depends on itself in order to be built if you want to be able to bootstrap from source.
...
Nix-like package management side-steps this issue by having explicit for dependencies of each package, internally uniquely naming package versions and independently linking the dependencies of each package.
...
So in order to package a piece of software you have to give the derivation everything needed to build the software as an input and the build is performed in an isolated environment where it cannot access anything not explicit included as an input in the build.
